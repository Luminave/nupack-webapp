...
### 🎨 VARNA结构导出（重点更新）
**功能说明**：集成VARNA引擎，支持导出发表级的RNA/DNA二级结构图。
...
- 🖥️ **桌面启动脚本** - 双击桌面图标即可启动
- 🔄 **自动打开浏览器** - 启动后自动打开操作面板
- ☕ **Java自动安装** - 自动检测并安装OpenJDK 11
- 📐 **布局优化** - 左右面板比例改为1:2，输入框更宽
- 🔧 **多链VARNA修复** - 正确处理多链结构的分隔符
...
### 🔄 自动布局（重点更新）
**算法原理**：核苷酸按序列顺序均匀分布在圆周上作为初始位置，然后通过 D3.js 力导向模拟，利用氢键的吸引力将核苷酸收缩到真实配对位置。配合共价键的约束力和节点间的排斥力，实现自动化的美观布局。

> 💡 **提示**：拖拉一下松紧滑块，结构更规整！

### 其他功能更新
- 📦 **紧凑布局**（不稳定） - 所有氢键和共价键等长，呈现规整网格
- 🔒 **确定布局**（不稳定） - 锁定后禁止拖拽核苷酸
- 🔃 **旋转功能**（不稳定） - 结构支持90度旋转
- 🎨 **多链模式** - 不同链使用不同颜色，支持自定义
- 🔍 **缩放改进** - 支持无限缩放，数字输入无限制
- 📏 **核苷酸大小** - 支持自定义核苷酸圆大小，数字输入无限制
- 🖊️ **线宽调节** - 支持自定义键线宽，数字输入无限制
- 💾 **下载项目** - 一键下载项目ZIP包
- ☕ **捐赠入口** - 支持开发者
- 📝 **版本号显示** - 标题栏显示当前版本
- ⚠️ **警告提示** - 提醒项目仍在开发中
